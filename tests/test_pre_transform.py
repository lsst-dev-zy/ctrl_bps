...
class TestCreatingQuantumGraph(unittest.TestCase):
    ...
    def testSuccess(self):
        ...
        _, cmd = config.search("createQuantumGraph", opt={"curvals": {"qgraphFile": qgraph_filename}})
        self.assertRegex(cm.output[0], cmd)
    ...
    def testCommandMissing(self):
        ...
        self.assertRegex(str(cm.exception), "command.*not found")
...
class TestUpdatingQuantumGraph(unittest.TestCase):
    ...
    def testSuccess(self):
        ...
        _, cmd = config.search("updateQuantumGraph", opt={"curvals": {"qgraphFile": str(self.src)}})
        self.assertRegex(cm.output[0], r"[Bb]acking up")
        self.assertRegex(cm.output[1], r"[Cc]ompleted")
        self.assertRegex(cm.output[2], cmd)
